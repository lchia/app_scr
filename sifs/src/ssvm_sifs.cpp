...
ssvm_sifs::ssvm_sifs(const std::string& input_fn,
    ...
    trainingLogFile << "   ->sdm::load_libsvm_binary(X_, y_, input_fn); " << endl;
    ...
    trainingLogFile << "   ->X_[i,:] = X_[i,:] * y_[i] " << endl;
    for (int i = 0; i < X_.rows(); ++i){
        X_.row(i) *= y_[i];
    }
    X_CM_ = X_;
    ...
    trainingLogFile << "   ->X_CM_ = X_;" << endl;
    trainingLogFile << "   ->n_sams_ = " << n_sams_ << endl;
    trainingLogFile << "   ->inv_n_sams_ = " << inv_n_sams_ << endl;
    trainingLogFile << "   ->n_feas_ = " << n_feas_ << endl;
    trainingLogFile << "   ->gamma_ = " << gamma_ << endl;
    trainingLogFile << "   ->inv_gamma_ = " << inv_gamma_ << endl;
    trainingLogFile << "   ->alpha_= " << alpha_ << endl;
    trainingLogFile << "   ->inv_alpha_= " << inv_alpha_ << endl;

    one_over_XTones_ =
        inv_n_sams_ * (X_CM_.transpose() * Eigen::VectorXd::Ones(n_sams_)).array();
    trainingLogFile << "   ->A01: one_over_XTones = 1/n * sum(X_CM_[i,:])_i=1:n (sum n samples) >>" << one_over_XTones_.size() << endl;

    // if beta > beta_max, we always have naive solutions
    trainingLogFile << "   =>A02: Compute beta_max_= one_over_XTones_.abs().maxCoeff();" << endl;
    trainingLogFile << "                        one_over_XTones_.abs()  >>" << one_over_XTones_.abs().size() << endl;
    beta_max_ = one_over_XTones_.abs().maxCoeff();
    trainingLogFile << "     >>beta_max_= " << beta_max_ << endl;
    trainingLogFile << "    ->beta_= " << beta_ << endl;
    ...
    trainingLogFile << "   =>A04: Compute alpha_max_(beta_) --" << endl;
    alpha_max_ = (1.0/(1.0 - gamma_)) * (X_ * temp.matrix()).maxCoeff();
    trainingLogFile << "     >>alpha_max_= " << alpha_max_ << endl;
    trainingLogFile << "     ->alpha_= " << alpha_ << endl;
    // if alpha > alpha_max, we have closed form solutions

    
    trainingLogFile << "   =>A05: Theorem 3 : If alpha > alpha_max_,  we have closed form solutions." << endl;
    trainingLogFile << "     	  ref_alpha_ = max{alpha_max_, 0}" << endl;
    if (alpha_max_ > 0) {
    	trainingLogFile << "     	If alpha_max_ > 0, we have formular (1)." << endl;
    	...
    	trainingLogFile << "     	If alpha_max_ < 0, we have all ref_dsol_ are 0 since line 1 of Algorithm 1." << endl;
        ref_alpha_ = 0;
        ref_dsol_ = Eigen::VectorXd::Ones(n_sams_);
        ref_psol_ = Eigen::VectorXd::Zero(n_feas_); //Why? -> line 1 of Algorithm 1. alpha_max = 0 > alpha_1 >alpha_2 >.. >alpha_M >0
...
void ssvm_sifs::compute_dual_obj(const bool& flag_comp_XTdsol) {
    //trainingLogFile << " ** compute_dual_obj -> (P*) **" << endl;
    if (flag_comp_XTdsol){
        update_psol(true);
    }
...
void ssvm_sifs::compute_duality_gap(const bool& flag_comp_loss,
    ...
    //trainingLogFile << " ** compute_duality_gap **" << endl;
...
void ssvm_sifs::update_psol(const bool& flag_comp_XTdsol) {
    trainingLogFile << "====================================================update psol_ ** " << std::endl;
    if (flag_comp_XTdsol) {
        XTdsol_.setZero();
        for (int i = 0; i < n_sams_; ++i) {
            if (dsol_[i] > 0.0) {
...
void ssvm_sifs::train_sifs(const int& scr_option) {
    ...
    trainingLogFile << "====================================================compute psol_ ** " << std::endl;
    compute_duality_gap(true, false);
    ...
    std::default_random_engine rg;
    std::uniform_int_distribution<> uni_dist(0, n_Dc - 1);//generate uniform random number in [0, n_Dc-1]
    //trainingLogFile << " <<<n_Dc = " << n_Dc << endl;

    const auto ins_begin_it = std::begin(idx_Dc_vec_);//
    auto random_it = std::next(ins_begin_it, uni_dist(rg));
    //trainingLogFile << " <<<The begin idx of the sample screening: ins_begin_it = " << *ins_begin_it << endl;
    //trainingLogFile << "   > Generate numbers in the range [0, n_Dc-1] : " << *random_it << endl;

 
    for (iter_ = 1; iter_ < max_iter_ && duality_gap_ > tol_; ++iter_) {
	if ((iter_-1)%50==0) {
            trainingLogFile << "      <<<n_Dc = " << n_Dc;
            trainingLogFile << " >iter: " << iter_;
            trainingLogFile << " >random_it: ";
	    log_ = 1;
	}
	else {
	    log_ = 0;
        ...
        if (iter_ % chk_fre_ == 0) {
            compute_duality_gap(true, false);
	    if(log_) {
                trainingLogFile << " >(" << iter_ << ")->duality_gap: " << get_duality_gap() << std::endl;
	    }
...
void ssvm_sifs::sample_screening(void) {
    ...
    //trainingLogFile << "  ***Radius of the ball where the primal optimum lies in -> (3)***" << endl;
    double psol_radius =
...
void ssvm_sifs::feature_screening(void) {
    if(log_) { trainingLogFile << "*IFS*"; }
    auto start_time = sys_clk::now();

    //trainingLogFile << "  ***Radius of the ball where the dual optimum lies in -> (6)***" << endl;
    double dsol_radius =
        std::sqrt(std::max(0.0, dif_alpha_ratio_ * dif_alpha_ratio_ *
