...
{
    trainingLogFile.open("trainingLog.txt", ios::out);
    trainingLogFile << "[1]************SIFS***************" << endl;

    
    trainingLogFile << "[2]parse_command_line(argc, argv); " << endl;
    ...
    trainingLogFile << "[3]ssvm_sifs solver(..); " << endl;
    ...
    trainingLogFile << "  nbs : " << nbs << std::endl;
    trainingLogFile << "  rbl : " << rbl << std::endl;
    trainingLogFile << "  rbu : " << rbu << std::endl;
    trainingLogFile << "  beta_lb = beta_max * rbl : " << beta_lb << std::endl;
    trainingLogFile << "  beta_ub = beta_max * rbu : " << beta_ub << std::endl;
    trainingLogFile << "  beta_log_inter = : " << beta_log_inter << std::endl;
    double beta_now;
    
    for (int i = nbs - 1; i >= 0; --i) {
	//selet beta_now according to (*beta) in section 5
	...
	trainingLogFile << "  beta_now = " << beta_now << "; (*beta) in section 5." << endl << endl;
        ...
        trainingLogFile << "    nas : " << nas << std::endl;
        trainingLogFile << "    ral : " << ral << std::endl;
        trainingLogFile << "    rau : " << rau << std::endl;
        trainingLogFile << "    alpha_lb = alpha_max * rbl : " << alpha_lb << std::endl;
        trainingLogFile << "    alpha_ub = alpha_max * rbu : " << alpha_ub << std::endl;
        trainingLogFile << "    alpha_log_inter = : " << alpha_log_inter << std::endl;
        double alpha_now;
